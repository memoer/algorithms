...
/**
 * 1. 자카드 유사도 J(A,B) -> A,B와 교집합 / A,B의 합집합 Ex) A->{1,2,3}, B->{2,3,4} => 2/4 => J(A,B) = 0.5
 * <p>
 * 2. 확장 자카드 유사도 J(A,B) Ex) A->{1,1,2,2,3}, B->{1,2,2,4,5}=>합집합->{1,1,2,2,3,4,5}, 교집합->{1,2,2}=>3/7 ...합집합할 경우, A는 1이 2개
 * / B는 1이 1개 -> 제일 큰 값 2개 -> 1,1 2개 넣는다.
 * <p>
 * Ex) FRANCE, FRENCH 2개씩 끊는다면, ...A -> {FR, RA, AN, NC, CE}, B -> {FR, RE, EN, NC, CH} ...합집합 -> {FR, RA, AN, NC, CE,
 * RE, EN, CH} ...교집합 -> {FR, NC}
 */
public class P17677 {
